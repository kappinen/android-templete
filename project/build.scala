...
object Settings {
  ...
  lazy val desktop = core ++ Seq(
    libraryDependencies ++= Seq(
      ...
      "com.badlogicgames.gdx" % "gdx-controllers-desktop" % libgdxVersion.value,
      "com.badlogicgames.gdx" % "gdx-controllers-platform" % libgdxVersion.value classifier "natives-desktop"
    ),
    fork in Compile := true,
    unmanagedResourceDirectories in Compile += file("android/assets"),
    desktopJarName := "bathfighter",
    Tasks.assembly
  )
  ...
  lazy val android = core ++ Tasks.natives ++ androidBuild ++ Seq(
    libraryDependencies ++= Seq(
      ...
      "com.badlogicgames.gdx" % "gdx-controllers" % libgdxVersion.value,
      "com.badlogicgames.gdx" % "gdx-controllers-android" % libgdxVersion.value
    ),
